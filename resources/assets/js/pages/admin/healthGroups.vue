        ...
        <div class="row">
            <div class="col-md-12">
                            ...
                            <div class="form-group" :class="{ 'has-error': errorsHealthGroups.length }">
                                <div class="form-row">
                                    <div class="col-md-10 col-xs-9">
                                        <input type="text" class="form-control" placeholder="Введите мед. группу здоровья"
                                            v-model="healthGroup"
                                        >
                                        <div v-for="(error, key) in errorsHealthGroups" :key="key" class="help-block">{{ error }}</div>
                                    </div>
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <form action="">
                            <div class="form-group" :class="{ 'has-error': errors[healthGroup.id].length }" v-for="healthGroup in healthGroups" :key="healthGroup.id">
                                <div class="form-row">
                                    <div class="col-md-10 col-xs-9">
                                        <input type="text" class="form-control" placeholder="Введите мед. группу здоровья"
                                            v-model="healthGroup.name"
                                        >
                                        <div v-for="(error, key) in errors[healthGroup.id]" :key="key" class="help-block">{{ error }}</div>
                                    </div>
                                    <div class="col-md-2 col-xs-3">
...
<script>
    ...
    export default {
        ...
        methods: {
            async save() {
                try {
                    let {data} = await axios.post('/api/health-group', {name: this.healthGroup})
            ...
            async update(id) {
                try {
                  ...
                  await axios.patch(`/api/health-group/${id}`, { name: newHealthGroup.name })
