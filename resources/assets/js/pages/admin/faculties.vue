        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary mb-3">
                    <div class="box-header with-border"><h3 class="box-title">Факультеты</h3></div>
                    <div class="box-body">
                        <form action="">
                            <div class="form-group" :class="{ 'has-error': errorsFaculty.length }">
                                <div class="form-row">
                                    <div class="col-md-10 col-xs-9">
                                        <input type="text" class="form-control" placeholder="Введите имя факультета" v-model="faculty">
                                        <div v-for="(error, key) in errorsFaculty" :key="key" class="help-block">{{ error }}</div>
                                    </div>
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <form action="">
                            <div class="form-group" :class="{ 'has-error': errors[faculty.id].length }" v-for="faculty in faculties" :key="faculty.id">
                                <div class="form-row">
                                    <div class="col-md-10 col-xs-9">
                                        <input type="text" class="form-control" placeholder="Введите имя факультета"
                                            v-model="faculty.name"
                                        >
                                        <div v-for="(error, key) in errors[faculty.id]" :key="key" class="help-block">{{ error }}</div>
                                    </div>
                                    <div class="col-md-2 col-xs-3">
                                        <div class="btn-group w-100">
                                            <button  class="btn d-block btn-primary w-50" type="button"
                                                @click.prevent="update(faculty.id)"
                                            ><i class="fa fa-refresh fa-fw"></i></button>
                                            <button class="btn d-block btn-danger w-50" type="button"
...
export default {
  ...
  methods: {
    async save () {
      try {
        const { data } = await axios.post('/api/faculty', { name: this.faculty })
