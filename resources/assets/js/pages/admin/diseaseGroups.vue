            ...
            <form action>
                ...
                <div class="form-row">
                  <div class="col-md-10 col-xs-9">
                    <input
                      type="text"
                      class="form-control"
                      placeholder="Введите название группы заболеваний"
                      v-model="diseaseGroup.name"
                    />
                    <div
                      v-for="(error, key) in errorsDiseaseGroups.name"
                      :key="key"
                      class="help-block"
                    >{{ error }}</div>
                  </div>
              ...
              <div class="form-group" :class="{ 'has-error': errorsDiseaseGroups.text.length }">
                <textarea
                  ...
                  rows="3"
                  v-model="diseaseGroup.text"
                  placeholder="Введите текст группы заболеваний"
                ></textarea>
                <div
                  v-for="(error, key) in errorsDiseaseGroups.text"
                  :key="key"
                  class="help-block"
                >{{ error }}</div>
              </div>
          ...
          <div class="box-body">
            <form action>
              <div v-for="diseaseGroup in diseaseGroups" :key="diseaseGroup.id" class="diseaseItem">
                <div
                  class="form-group"
                  :class="{ 'has-error': errors[diseaseGroup.id].name.length }"
                >
                  <div class="form-row">
                    <div class="col-md-10 col-xs-9">
                      <input
                      ...
                      />
                      <div
                        v-for="(error, key) in errors[diseaseGroup.id].name"
                        :key="key"
                        class="help-block"
                      >{{ error }}</div>
                    </div>
                ...
                <div
                  class="form-group"
                  :class="{ 'has-error': errors[diseaseGroup.id].text.length }"
                >
                  <textarea class="form-control v-resize" rows="3" v-model="diseaseGroup.text"></textarea>
                  <div
                    v-for="(error, key) in errors[diseaseGroup.id].text"
                    :key="key"
                    class="help-block"
                  >{{ error }}</div>
                </div>
...
export default {
  ...
  metaInfo() {
    return { title: "Группы заболевания" };
  },
  ...
  created() {
    (async () => {
      try {
        ...
        data.map(item =>
          this.$set(this.errors, item.id, { name: [], text: [] })
        );
      } catch (errors) {
        console.log(errors);
      }
  ...
  methods: {
    async save() {
      try {
        ...
        IziToast.success({
          message: "Successfully saved new medical disease group!"
        });
    ...
    async update(id) {
      try {
        const newDiseaseGroup = this.diseaseGroups.find(
          value => value.id === id
        );
        await axios.patch(`/api/disease-group/${id}`, newDiseaseGroup);

        IziToast.success({
          message: "Successfully updated medical disease group!"
        });
    ...
    async remove(id) {
      try {
        ...
        IziToast.success({
          message: "Successfully removed medical disease group!"
        });
      } catch (errors) {
        console.log(errors);
      }
